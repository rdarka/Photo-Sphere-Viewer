import { ACTIONS, DBLCLICK_DELAY, IDS, INERTIA_WINDOW, MOVE_THRESHOLD } from '../data/constants';
...
class PSVEventsHandler extends AbstractService {
  ...
  __fullscreenToggled() {
     ...
     */
    this.psv.trigger('fullscreen-updated', enabled);
  }
    ...
    if (intersect) {
      ...
      if (!this.state.dblclickTimeout) {
        ...
        if (Math.abs(this.state.dblclickData.clientX - data.clientX) < MOVE_THRESHOLD
           ...
           * @param {PhotoSphereViewer.ClickData} data
           */
          this.psv.trigger('dblclick', this.state.dblclickData);
        }
