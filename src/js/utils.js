...
export function isInteger(value) {
  ...
  /* eslint-disabled-next-line no-restricted-global */
 ...
 * @returns {*}
 */
export function getStyle(elt, pseudoElt, prop) {
  if (prop === undefined) {
    return window.getComputedStyle(elt, null)[pseudoElt];
  }
  else {
    return window.getComputedStyle(elt, pseudoElt)[prop];
  }
}
 ...
 * @param {external:THREE.Scene} scene
 */
export function cleanTHREEScene(scene) {
    ...
    if (item instanceof THREE.Mesh) {
      ...
      if (item.material) {
        if (Array.isArray(item.material)) {
          ...
          if (item.material instanceof THREE.MeshBasicMaterial && item.material.map) {
