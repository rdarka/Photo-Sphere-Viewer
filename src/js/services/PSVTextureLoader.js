...
class PSVTextureLoader extends AbstractService {
   ...
   * @returns {Promise.<external:THREE.Texture|external:THREE.Texture[]>}
   ...
   * @returns {Promise.<external:THREE.Texture>}
   * @fires module:services.PSVTextureLoader.panorama-load-progress
   * @throws {PSVError} when the image cannot be loaded
   * @private
   */
  __loadEquirectangularTexture(panorama) {
    ...
    return this.__loadXMP(panorama)
      .then(panoData => new Promise((resolve, reject) => {
        ...
        const onload = (img) => {
           ...
           */
          this.psv.trigger('panorama-load-progress', panorama, progress);

          if (panoData) {
            this.config.panoData = panoData;
          }
          else {
          ...
          const ratio = Math.min(this.config.panoData.fullWidth, SYSTEM.maxTextureWidth) / this.config.panoData.fullWidth;

          // resize image / fill cropped parts with black
          if (ratio !== 1
            || this.config.panoData.croppedWidth !== this.config.panoData.fullWidth
            || this.config.panoData.croppedHeight !== this.config.panoData.fullHeight
          ) {
            const resizedPanoData = clone(this.config.panoData);
          ...
          if (this.config.cacheTexture) {
            this.__putPanoramaCache({
              panorama: panorama,
              image   : texture,
              panoData: clone(this.config.panoData),
            });
          }
  ...
  __loadCubemapTexture(panorama) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onload = (i, img) => {
        ...
        this.psv.trigger('panorama-load-progress', panorama[i], progress[i]);
      ...
      const onprogress = (i, e) => {
        if (e.lengthComputable) {
          ...
          if (newProgress > progress[i]) {
            ...
            this.psv.oader.setProgress(sum(progress) / 6);
            this.psv.trigger('panorama-load-progress', panorama[i], progress[i]);
          }
        }
      };
  ...
  __putPanoramaCache(cache) {
     ...
     * @param {string} panorama
     */
    this.psv.trigger('panorama-cached', cache.panorama);
  }
